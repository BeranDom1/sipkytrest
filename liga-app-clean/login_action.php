...
function safe_next(string $url, string $default, string $base) : string {
    // dovol jen interní cesty pod aktuální aplikací
    $u = parse_url($url);
    if (!$u || !empty($u['scheme']) || !empty($u['host'])) return $default;
    if (empty($u['path']) || strpos($u['path'], $base) !== 0) return $default;
    return $url;
}
...
$next     = safe_next($_POST['next'] ?? '', $BASE_URL.'/admin/index.php', $BASE_URL);
