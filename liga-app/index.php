...
if ($rocnik_id !== 1) {
    $st = $conn->prepare("SELECT 1 FROM zapasy WHERE rocnik_id = ? AND $validScoreSql AND (average_home IS NOT NULL OR average_away IS NOT NULL) LIMIT 1");
    $st->bind_param('i', $rocnik_id);
    $st->execute();
...
if ($showTops) {
    ...
    $st = $conn->prepare("SELECT u.jmeno, ROUND(AVG(a.val),2) AS avg_val FROM (
        SELECT hrac1_id AS hid, average_home AS val FROM zapasy WHERE rocnik_id=? AND average_home IS NOT NULL AND $validScoreSql
        UNION ALL
        SELECT hrac2_id AS hid, average_away AS val FROM zapasy WHERE rocnik_id=? AND average_away IS NOT NULL AND $validScoreSql
    ...
    $st = $conn->prepare("SELECT u.jmeno, MAX(a.val) AS hf FROM (
        SELECT hrac1_id AS hid, high_finish_home AS val FROM zapasy WHERE rocnik_id=? AND $validScoreSql
        UNION ALL
        SELECT hrac2_id AS hid, high_finish_away AS val FROM zapasy WHERE rocnik_id=? AND $validScoreSql
    ...
    $st = $conn->prepare("SELECT u.jmeno, SUM(a.body) AS body FROM (
        SELECT hrac1_id AS hid, COALESCE(count_100p_home,0)*1 + COALESCE(count_120p_home,0)*2 + COALESCE(count_140p_home,0)*3 + COALESCE(count_160p_home,0)*4 + COALESCE(count_180_home,0)*5 AS body FROM zapasy WHERE rocnik_id=? AND $validScoreSql
        UNION ALL
        SELECT hrac2_id AS hid, COALESCE(count_100p_away,0)*1 + COALESCE(count_120p_away,0)*2 + COALESCE(count_140p_away,0)*3 + COALESCE(count_160p_away,0)*4 + COALESCE(count_180_away,0)*5 AS body FROM zapasy WHERE rocnik_id=? AND $validScoreSql
...
if ($hasLigaNazvy) {
    $st = $conn->prepare("SELECT ln.liga_id AS id, ln.nazev, SUM(
      CASE WHEN $validJoinedScoreSql THEN 1 ELSE 0 END
