...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check($_POST['csrf'] ?? '')) {
        ...
        if ($u === '' || $p === '' || $p2 === '') {
            ...
            // upsert (pokud username existuje, aktualizuj heslo)
            $stmt = $conn->prepare("
            ...
            if ($stmt->execute()) {
                $msg = "Admin „".htmlspecialchars($u)."“ vytvořen/aktualizován. 
                        <a href=\"{$BASE_URL}/login.php?next=" . urlencode($BASE_URL.'/admin/index.php') . "\">Přejít na přihlášení</a>";
            } else {
                $err = 'Chyba při ukládání: '.$conn->error;
            }
        }
    }
}
...
<div class="box">
  ...
  <p style="font-size:.9rem;color:#666;margin-top:.75rem">
    Po úspěchu tenhle soubor <b>ihned smaž</b>.
  </p>
</div>
</body>
</html>
