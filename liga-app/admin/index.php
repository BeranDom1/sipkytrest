...
<body class="admin-body"><main class="admin-shell">
  ...
  <h1 class="admin-title">Administrace</h1><p class="admin-subtitle"><?php if ($portalRole === 'admin'): ?>Aktivní sezóna: <strong><?= htmlspecialchars($active['nazev'] ?? 'není nastavena') ?></strong><?php else: ?>Vyberte část, kterou chcete spravovat.<?php endif; ?></p>
  <?php if ($portalRole === 'admin'): ?>
  ...
  <div class="admin-grid admin-grid--two" style="margin-top:14px">
    ...
    <section class="admin-card"><h2>Turnaje</h2><div class="admin-actions"><a class="admin-btn admin-btn--secondary" href="/liga-app/admin/registrace-turnaje.php">Registrace na turnaje</a></div></section>
  </div>
  <?php endif; ?>
</main></body></html>
