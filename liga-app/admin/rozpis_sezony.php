...
<div class="admin-league-grid"><?php foreach ($leagueData as $data): $count=count($data['players']); $actual=count($data['matches']); $ok=$count>=2 && $data['schedule_valid']; ?><section class="admin-card"><h2><?= htmlspecialchars($data['league']['nazev']) ?></h2><p><strong><?= $count ?> hráčů</strong> · očekáváno <?= $data['expected'] ?> zápasů · uloženo <?= $actual ?></p><div class="admin-check <?= $ok?'admin-check--ok':'admin-check--bad' ?>"><?= $ok?'✓ Konzistentní':'⚠ Vyžaduje opravu' ?><?= $count % 2 === 1 ? ' · v každém kole má jeden hráč volno' : '' ?></div><ol class="admin-player-list"><?php foreach ($data['players'] as $player): ?><li><?= htmlspecialchars($player['jmeno']) ?></li><?php endforeach; ?></ol></section><?php endforeach; ?></div>
