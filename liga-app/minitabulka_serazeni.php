...
// Fallback při absenci vzájemných zápasů: RZD (desc) -> Jméno (asc)
        ...
        if (count($skupina) <= 2) {
            usort($skupina, function($a, $b) use ($conn, $rocnik_id, $liga_id) {
                $sql = "SELECT hrac1_id, hrac2_id, skore1, skore2
                        ...
                        LIMIT 1";
                if ($liga_id !== null) {
                    // vlož podmínku na ligu
                    $sql = str_replace("LIMIT 1", "AND liga_id = ? LIMIT 1", $sql);
                    $st  = $conn->prepare($sql);
                ...
                if ($m) {
                    if     ($m['hrac1_id'] == $a['player_id'] && $m['skore1'] > $m['skore2']) return -1; // a vyhrál
                    elseif ($m['hrac2_id'] == $a['player_id'] && $m['skore2'] > $m['skore1']) return -1; // a vyhrál
                    elseif ($m['skore1'] != $m['skore2']) return 1; // b vyhrál
                }
            ...
            if ($pocetMinitabulkovychZapasu === 0) {
                // --- žádné vzájemné zápasy v téhle skupině → Fallback na RZD (desc), pak Jméno
                usort($skupina, function($a, $b) {
                    if (($a['RZD'] ?? 0) !== ($b['RZD'] ?? 0)) return ($b['RZD'] ?? 0) <=> ($a['RZD'] ?? 0);
                    return strcasecmp($a['jmeno'], $b['jmeno']);
                });
            } else {
                // --- standardní minitabulka: počet výher -> mini RZD -> jméno
                usort($skupina, function($a, $b) {
